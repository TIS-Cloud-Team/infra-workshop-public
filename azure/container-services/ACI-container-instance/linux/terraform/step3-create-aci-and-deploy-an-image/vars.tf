...
locals {
  ...
  ## acr & aci name
  azure_existing_resource_group_name = "rg-<name>"
  image_name = "workshop-dotnetapp" ##this dotnet core aspnet app runs on port 8080 (linux compatible) 
  aci_group_name = replace("aci-group-${local.image_name}","_", "-")
  aci_name = replace("aci-${local.image_name}","_", "-")

  ##azure_container_name = "acr${local.app_name}${local.current_datetime}"
  azure_container_registry_login_server = "<acrname>.azurecr.io"
  azure_container_registry_admin_username = "<acr-username>"
  azure_container_registry_admin_password = "<acr-user-password>>"
  ## mcr.microsoft.com/azuredocs/aci-helloworld
  azure_image_name = "${local.azure_container_registry_login_server}/${local.image_name}:latest"
